...
# GLOBAL VARIABLES
timer_period = 0.3 # seconds  DON'T CHANGE THIS VALUE
...
def cost_function(z_i, sigma, r_i, gamma,a,b,epsilon):
  ...
  Estimated variables:
    ...
    a, b , epsilon weights of the barrier function
  ...
  Cost function for agent i:
    ...
    r_i = target position
    sigma = barycenter of the agents
    barrier = -epsilon*log(-g(z_i)) = -epsilon*log(-a*z_i[0]^2 + b*z_i[1]^2 + 1)
  """
...
class Agent(Node):
    def __init__(self):
        ...
        self.ZZ[self.lower_index:self.upper_index,1] = np.array(self.z_i)
        ...
        f_i,_,df_i = cost_function(self.ZZ[self.lower_index:self.upper_index,0], self.SS[self.lower_index:self.upper_index,0], self.RR[self.lower_index:self.upper_index,0], self.gamma,self.a,self.b,self.epsilon)
    ...
    def timer_callback(self):
        ...
        if self.do_init:
            ...
            if all_received and self.t < self.MAXITERS-1:
                ...
                _, dfi_dxi_k, df_dsigma_k = cost_function(z_i, sigma_i, r_i,self.gamma,self.a,self.b,self.epsilon)
                ...
                f_i, _, df_dsigma_kp = cost_function(self.ZZ[self.lower_index:self.upper_index,self.t+1], self.SS[self.lower_index:self.upper_index,self.t+1], r_i,self.gamma,self.a,self.b,self.epsilon) 
